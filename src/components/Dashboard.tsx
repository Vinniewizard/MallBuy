...
import { Copy, ArrowRight, ShieldAlert, ShieldCheck, Sparkles, Coins, Flame, Gem, TrendingUp, HelpCircle, Wind, Droplets, Bot, Coffee, Package, ChevronDown, ChevronUp, Settings2, Eye, EyeOff } from "lucide-react";
...
export default function Dashboard({ user, stats, referrals, plans, balance, onShop, onSwitchTab, onRefresh }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    if (savedPrefs) {
      try {
        setWidgetPrefs(JSON.parse(savedPrefs));
      } catch (e) {
  ...
  const toggleWidget = (widgetKey: keyof typeof widgetPrefs) => {
    const newPrefs = { ...widgetPrefs, [widgetKey]: !widgetPrefs[widgetKey] };
  ...
  return (
    <div className="space-y-12 pb-8 relative">
      ...
      <div className="flex justify-end">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-colors ${
            isEditMode ? "bg-emerald-500 text-white" : "bg-white/5 text-slate-400 hover:text-white border border-white/10 hover:bg-white/10"
          }`}
        >
      ...
      {isEditMode && (
        <div className="bg-slate-900 border border-emerald-500/30 rounded-2xl p-6 mb-8 shadow-xl">
          <h3 className="text-sm font-extrabold text-white mb-4">Dashboard Layout</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <button onClick={() => toggleWidget('showIntro')} className={`p-4 rounded-xl border flex items-center justify-between transition-colors ${widgetPrefs.showIntro ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10'}`}>
              <span className="text-xs font-bold">Intro Message</span>
              {widgetPrefs.showIntro ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            </button>
            <button onClick={() => toggleWidget('showStats')} className={`p-4 rounded-xl border flex items-center justify-between transition-colors ${widgetPrefs.showStats ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10'}`}>
              <span className="text-xs font-bold">Account Stats</span>
              {widgetPrefs.showStats ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            </button>
            <button onClick={() => toggleWidget('showQuickActions')} className={`p-4 rounded-xl border flex items-center justify-between transition-colors ${widgetPrefs.showQuickActions ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10'}`}>
              <span className="text-xs font-bold">Quick Actions</span>
              {widgetPrefs.showQuickActions ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            </button>
            <button onClick={() => toggleWidget('showWholesale')} className={`p-4 rounded-xl border flex items-center justify-between transition-colors ${widgetPrefs.showWholesale ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10'}`}>
              <span className="text-xs font-bold">Wholesale Desk</span>
              {widgetPrefs.showWholesale ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            </button>
            <button onClick={() => toggleWidget('showMarketPulse')} className={`p-4 rounded-xl border flex items-center justify-between transition-colors ${widgetPrefs.showMarketPulse ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10'}`}>
              <span className="text-xs font-bold">Market Pulse</span>
              {widgetPrefs.showMarketPulse ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            </button>
          </div>
        </div>
      )}
      ...
      {/* Intro section */}
      {(widgetPrefs.showIntro || isEditMode) && (
        <div className={`transition-opacity ${!widgetPrefs.showIntro && isEditMode ? 'opacity-30' : ''}`}>
      ...
      {(widgetPrefs.showStats || isEditMode) && (
        <div className={`bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-md transition-opacity ${!widgetPrefs.showStats && isEditMode ? 'opacity-30' : ''}`}>
          <button
      ...
      {(widgetPrefs.showQuickActions || isEditMode) && (
        <div className={`bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm backdrop-blur-md transition-opacity ${!widgetPrefs.showQuickActions && isEditMode ? 'opacity-30' : ''}`}>
      ...
      {/* WHOLESALE DESK */}
      {(widgetPrefs.showWholesale || isEditMode) && (
        <div className={`space-y-6 pt-4 transition-opacity ${!widgetPrefs.showWholesale && isEditMode ? 'opacity-30' : ''}`}>
          <div>
      ...
      {/* MARKET PULSE */}
      {(widgetPrefs.showMarketPulse || isEditMode) && (
        <div className={`space-y-6 pt-6 transition-opacity ${!widgetPrefs.showMarketPulse && isEditMode ? 'opacity-30' : ''}`}>
          <div>
